...
mEarth = 3.2e19  # mass of Earth
mCraft = 175 # mass of spacecraft

# Objects and initial values
Earth = sphere(pos=vec(0,0,0), radius = REarth, 
               m = mEarth, color=color.cyan)
Craft =  sphere(pos=Earth.pos + vec(REarth, 0, 0), 
          color=color.magenta,  
          make_trail = True,
          m = mCraft,
          radius  = REarth/12 )

v0 = vec(3,0,0)#vec(input("need x   "),input("need y   "),input("need z   "))
pCraft = mCraft * v0
vcr=pCraft/mCraft
...
if ctn=="y":
    while boopnt==0:
        ...
        if mag(r)<REarth: 
            ...
            pCraft = mCraft * v0
            vcr=pCraft/mCraft
            Craft.pos=Earth.pos + vec(REarth, 0, 0)
        else:
            print("escape'd")
            boopnt=1
    print("escvel   ",v0)
